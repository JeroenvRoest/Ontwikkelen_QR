...
function loadClientData(clientId) {
 //gegevens ophalen via API
    fetch(`https://1a2480b0-fd9b-48ad-bfa8-f417948043b0.mock.pstmn.io/t/client/${clientId}`)
    .then(res => {
    ...
    .then(data => {
      ...
      document.getElementById("showNotesBtn").onclick = () => fetchMedicalNotes(data.id);
      document.getElementById("showReportsBtn").onclick = () => fetchReportsByClient(data.id);
      document.getElementById("showEmergencyContactBtn").onclick = () => fetchEmergencyContact(data.id);
    })
    .catch(err => {
      console.error(err);
